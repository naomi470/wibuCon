...
async function v2rayToSing(v2rayAccount) {
  ...
  function parseVlessUrl(ftpArrayUrl2) {
    ...
    if (
      ftpParsedUrl.port === "443" ||
      ftpParsedUrl.searchParams.get("security") === "tls"
    ) {
      configResult.tls = {
        enabled: true,
    ...
    configResult.transport =
      transportTypes[ftpParsedUrl.searchParams.get("type")];
    return configResult;
  }
  ...
  function parseShadowsocksRUrl(ftpArrayUrl2) {
    let ftpParsedUrl = ftpArrayUrl2.substring(6);
    let decodeResult = gBase64.decode(ftpParsedUrl);
    let [serverSSR, portSSR, protocolSSR, methodSSR, obfsSSR, passwordSSR] =
      decodeResult.split(":");
...
async function parseUrl() {
  ...
  const urls = {
    ...
    nekobox:
      "https://raw.githubusercontent.com/iyarivky/sing-ribet/main/config/config-nekobox.json",
  };
  const configs = {};
  ...
  for (const name of configNames) {
    ...
    const servers = config.outbounds
      ...
      .filter((filterserver) => !ipChecker(filterserver));
    const directDnsRule = config.dns.rules.find(
      (rule) => rule.server === "direct-dns"
    );
    if (directDnsRule) {
